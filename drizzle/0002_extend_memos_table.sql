-- 扩展 memos 表以完全兼容 ContentItem 接口
-- 添加缺失的字段，保持向后兼容

-- 添加 ContentItem 核心字段
ALTER TABLE memos ADD COLUMN type TEXT NOT NULL DEFAULT 'memo';
...
UPDATE memos SET 
  ...
  metadata = '{}'
WHERE slug IS NULL OR source IS NULL OR file_path IS NULL OR publish_date IS NULL;

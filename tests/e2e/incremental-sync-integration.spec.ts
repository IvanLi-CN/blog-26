...
// 等待同步完成的辅助函数
const waitForSyncCompletion = async (page: any, context: string, _timeout = 30000) => {
  console.log(`⏳ [${context}] 等待增量同步完成...`);

  // 等待一段时间让同步操作完成
  // 注意：这里使用固定等待时间，在实际项目中可能需要更智能的等待策略
  await page.waitForTimeout(5000);

  console.log(`✅ [${context}] 同步等待完成`);
};
...
test.describe("增量数据同步功能集成测试", () => {
  ...
  test.describe("闪念系统增量同步测试", () => {
    test("创建闪念应该触发增量同步", async ({ page }) => {
      ...
      // 查找快速编辑器区域 - 使用更精确的选择器避免冲突
      const quickEditorSection = page.locator('section[data-testid="quick-memo-editor"]');
      await expect(quickEditorSection).toBeVisible({ timeout: 10000 });

      // 查找编辑器内的可编辑区域 - 使用更通用的选择器
      const editorContent = quickEditorSection.locator('[contenteditable="true"]').first();
      ...
      // 查找并点击发布按钮 - 使用更精确的选择器
      const publishButton = quickEditorSection.getByRole("button", { name: "发布 Memo" });
      await expect(publishButton).toBeEnabled({ timeout: 5000 });
      await publishButton.click();

      // 等待增量同步完成
      await waitForSyncCompletion(page, "MEMO-CREATE");
    ...
    test("编辑闪念应该触发增量同步", async ({ page }) => {
      ...
      if (editButtonCount > 0) {
        ...
        // 等待增量同步完成
        await waitForSyncCompletion(page, "MEMO-EDIT");
  ...
  test.describe("文章系统增量同步测试", () => {
    test("创建文章应该触发增量同步", async ({ page }) => {
      ...
      // 等待增量同步完成
      await waitForSyncCompletion(page, "ARTICLE-CREATE");
  ...
  test.describe("数据一致性验证", () => {
    test("验证管理员页面和前端页面的数据一致性", async ({ page }) => {
      ...
      // 获取管理员页面的文章列表 - 管理员页面使用表格结构
      const adminArticles = page.locator("tbody tr");
      const adminCount = await adminArticles.count();

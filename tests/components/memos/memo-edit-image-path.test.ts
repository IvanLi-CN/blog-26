...
describe('Memo编辑器图片路径转换测试', () => {
  describe('编辑器上传的图片路径', () => {
    it('应该正确处理assets/tmp/目录下的图片', async () => {
      const markdown = '![测试图片](assets/tmp/1754495759130_image.png)';
      const html = await parseMarkdownToHTML(markdown, 'Memos/20250806_yPWtUnKt.md');
      const src = extractImageSrc(html);

      expect(src).toBe('/api/render-image/assets/tmp/1754495759130_image.png?f=webp&q=85&s=1200&dpr=1');
    });

    it('应该正确处理内联图片路径', async () => {
      const markdown = '![image](assets/tmp/inline-1754495688640.png)';
      const html = await parseMarkdownToHTML(markdown, 'Memos/20250806_yPWtUnKt.md');
      const src = extractImageSrc(html);

      expect(src).toBe('/api/render-image/assets/tmp/inline-1754495688640.png?f=webp&q=85&s=1200&dpr=1');
    });

    it('应该正确处理带时间戳的文件名', async () => {
      const markdown = '![测试](assets/tmp/1754493684240_Memo-图片1.png)';
      const html = await parseMarkdownToHTML(markdown, 'Memos/20250806_yPWtUnKt.md');
      const src = extractImageSrc(html);

      expect(src).toBe('/api/render-image/assets/tmp/1754493684240_Memo-图片1.png?f=webp&q=85&s=1200&dpr=1');
    });

    it('应该正确处理包含特殊字符的文件名', async () => {
      const markdown = '![测试](assets/tmp/1754495473767_image (1).png)';
      const html = await parseMarkdownToHTML(markdown, 'Memos/20250806_yPWtUnKt.md');
      const src = extractImageSrc(html);

      expect(src).toBe('/api/render-image/assets/tmp/1754495473767_image (1).png?f=webp&q=85&s=1200&dpr=1');
    });
  });
  ...
  describe('验证实际的编辑器路径格式', () => {
    it('应该验证编辑器生成的路径格式是正确的', async () => {
      ...
      const editorPath = `assets/tmp/${filename}`;
      ...
      // 验证路径转换是否正确
      expect(src).toBe(`/api/render-image/assets/tmp/${filename}?f=webp&q=85&s=1200&dpr=1`);
    });
  ...
  describe('对比快速发布编辑器的路径', () => {
    it('快速发布和编辑器应该使用相同的路径格式', async () => {
      // 快速发布编辑器使用的路径格式
      const quickPublishPath = 'assets/tmp/1754495473767_quick-publish.png';
      // 编辑器使用的路径格式
      const editorPath = 'assets/tmp/1754495473767_editor-upload.png';

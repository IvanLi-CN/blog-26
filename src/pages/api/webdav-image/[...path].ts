...
export const GET: APIRoute = async ({ params, request }) => {
  try {
    ...
    if (normalizedPath.startsWith(`${cleanMemosPath}/`)) {
      ...
      console.log(`WebDAV image proxy: Using pre-formatted path: ${normalizedPath}`);
    } else {
    ...
    console.log(`WebDAV image proxy: Trying paths:`, allPossiblePaths);
    ...
    for (const tryPath of allPossiblePaths) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            `WebDAV image proxy: Successfully fetched image from path: ${tryPath}, content-type: ${contentType}, size: ${imageBuffer.byteLength} bytes`
          );
          ...
          console.log(`WebDAV image proxy: Failed to fetch from ${tryPath}: ${response.status} ${response.statusText}`);
        }
      } catch (error) {
        console.log(`WebDAV image proxy: Error trying path ${tryPath}:`, error.message);
      }
    }

    console.error(`WebDAV image proxy: Failed to find image in any location for path: ${path}`);

...
export const adminPostsRouter = createTRPCRouter({
  ...
  list: adminProcedure.input(getPostsSchema).query(async ({ input }) => {
    ...
    try {
      ...
      type PostWithVector = PostRow & { vectorizationStatus?: VectorizationStatus };
      const postsWithVector: PostWithVector[] = await Promise.all(
        postsList.map(async (p) => {
          ...
          const enriched: PostWithVector = { ...p, vectorizationStatus: status };
          return enriched;
        })
      );
  ...
  update: adminProcedure.input(updatePostSchema).mutation(async ({ input }) => {
    ...
    try {
      ...
      const extracted = extractPostDraftFields(nextContent, {
        ...
        slug: updateData.slug ?? existingPost.slug,
        excerpt: updateData.excerpt ?? existingPost.excerpt,
        draft: updateData.draft ?? existingPost.draft,
        public: updateData.public ?? existingPost.public,
      ...
      const updatedData = {
        ...
        author: extracted.author,
        image: extracted.image,
        publishDate: extracted.publishDate ?? existingPost.publishDate,
        updateDate: extracted.updateDate ?? Date.now(),
        contentHash,
        lastModified: Date.now(),
        filePath: existingPost.filePath || `blog/${extracted.slug}.md`,
      };

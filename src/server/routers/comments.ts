    ...
    .where(
      and(
        ...
        or(
          ...
          isAdmin ? undefined : eq(comments.authorEmail, currentUserEmail || "")
        )
      )
    )
...
export const commentsRouter = createTRPCRouter({
  ...
  // 批量获取评论数
  getCommentCounts: publicProcedure.input(getCommentCountsSchema).query(async ({ input }) => {
    const { slugs } = input;
  ...
  createComment: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!ctx.user && author) {
        ...
        // 查找或创建用户
        const user = await db.select().from(users).where(eq(users.email, authorEmail)).get();
  ...
  getAdminComments: adminProcedure.input(getAdminCommentsSchema).query(async ({ input }) => {
    ...
    if (search) {
      conditions.push(
        or(like(comments.content, `%${search}%`), like(comments.authorName, `%${search}%`))
      );
    }

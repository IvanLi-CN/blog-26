...
export function ScheduledJobsPanel() {
  ...
  return (
    <div className="space-y-10">
      <section className="rounded-box border border-base-200 bg-base-100 p-6 space-y-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-3xl font-bold">定时任务</h2>
        ...
        {runsLoading ? (
          ...
          <ul className="divide-y divide-base-200 rounded-box border border-base-200">
            {recentRuns.map((run) => (
              <li
                key={run.id}
                className="flex flex-wrap items-center gap-4 px-5 py-4 hover:bg-base-200/40 transition-colors"
              >

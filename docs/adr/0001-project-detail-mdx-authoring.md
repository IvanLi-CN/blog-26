...
The runtime exposes a small allowlist of static server-rendered React content blocks: `ProjectFigure`, `ProjectCallout`, `ProjectFacts`, and `ProjectComparison`. Build-time validation rejects arbitrary site-component imports and non-named imports from the content-block module. A new block requires the same need to appear in at least two projects. An MDX author may optionally wrap a narrative section in the semantic `project-mdx-section` class when separate reading surfaces help; the page never invents sections or cards automatically.

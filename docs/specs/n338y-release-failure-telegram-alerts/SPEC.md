# n338y · Release 失败 Telegram 告警接入

## Summary
- 为 `Release (PR Label Driven)` 工作流补一个 repo-local notifier wrapper，统一复用共享 Telegram 告警 workflow。
...
## Acceptance
- `workflow_run` 在 `Release (PR Label Driven)` 失败时触发 Telegram 告警。

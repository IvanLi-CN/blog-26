...
- File-tree rename failures now roll back into the inline input instead of dropping the operator out of edit mode, with a restrained danger-tinted field treatment and a persistent toast so the failure cause stays visible while the operator corrects the name.

...
4. Keep lower-case memo roots configurable for dev/test overrides when explicitly requested.
...
- Local memo helpers must normalize configured roots and derive:
  ...
  - relative file IDs
- Lower-case `/memos` remains supported only when explicitly configured.
...
- Keep explicit `LOCAL_MEMOS_PATH=/memos` override support for fallback scenarios.

...
The legacy MCP implementation mixed transport concerns with content write logic and did not distinguish MCP-authored content from ordinary local storage rows. The replacement keeps the endpoint stable while aligning with current Streamable HTTP session semantics and adding a separate authoring-origin marker.

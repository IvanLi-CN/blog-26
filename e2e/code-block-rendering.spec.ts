...
test.describe("Code Block Rendering", () => {
  ...
  test("should not have code-block related hydration errors", async ({ page }) => {
    ...
    // Check if there are any code-block specific hydration errors
    const codeBlockHydrationErrors = relevantErrors.filter(
      (error) =>
        (error.includes("hydration") ||
          error.includes("Hydration") ||
          error.includes("server rendered HTML") ||
          error.includes("client properties")) &&
        (error.includes("code") || error.includes("Code") || error.includes("[object Object]"))
    );

    // We expect no code-block specific hydration errors
    expect(codeBlockHydrationErrors.length).toBe(0);

    // Log any remaining hydration errors for debugging (but don't fail the test)
    const generalHydrationErrors = relevantErrors.filter(
      (error) =>
        error.includes("hydration") ||
        error.includes("Hydration") ||
        error.includes("server rendered HTML") ||
    ...
    if (generalHydrationErrors.length > 0) {
      console.log(
        "Note: General hydration errors detected (not code-block related):",
        generalHydrationErrors
      );
    }
  });

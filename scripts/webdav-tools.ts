...
async function getFileContent(filePath: string, showMetadata: boolean = false): Promise<void> {
  ...
  try {
    ...
    if (showMetadata) {
      // 获取文件元数据
      try {
        // 使用 listFiles 获取父目录信息，然后找到目标文件
        const parentPath = filePath.substring(0, filePath.lastIndexOf("/")) || "/";
        const fileName = filePath.substring(filePath.lastIndexOf("/") + 1);
        const files = await webdavClient.listFiles(parentPath);
        const fileInfo = files.find((f: any) => f.basename === fileName);

        if (fileInfo) {
          console.log("📋 文件信息:");
          console.log(`   📁 文件名: ${fileInfo.basename}`);
          console.log(`   📏 大小: ${formatFileSize(fileInfo.size)}`);
          console.log(`   📅 修改时间: ${fileInfo.lastmod}`);

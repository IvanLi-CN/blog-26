...
usage() {
  cat <<USAGE
Usage: ./scripts/setup.sh [--with-db] [--with-e2e] [--dry-run]

Options:
  --with-db     Reset dev DB and seed dev data
...
main() {
  ...
  # 4) Optional: local DB and dev data
  ...
  log "Done. Next: 'bun run dev' (WebDAV + Next.js). Use --with-db to pre-seed."
}
